...
export function analyticalHistory(state) {
  const completed = new Set(uniqueCompletions(state));
  return state.history.filter(row => row.status !== 'completed' || completed.has(row));
}
