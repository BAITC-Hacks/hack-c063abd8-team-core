import { normalizeDataset, compileRequirements, reconcileProgress, asOf, completionKey, analyticalHistory } from './dataset.js';
...
export function remainingSessions(event, employee, state) {
  const completed = new Set(state.history.filter(h => h.employee_id === employee.employee_id && h.event_id === event.event_id && h.status === 'completed').map(h => h.session_date || h.date.slice(0, 10)));
  const dates = event.format === 'self_paced' && event.recurring ? [asOf(state).slice(0, 10)] : event.upcoming_sessions || [];
  return [...new Set(dates)].filter(d => d >= asOf(state).slice(0, 10) && (!event.recurring || !completed.has(d))).sort();
}
export function eligible(event, employee, state) {
  if (event.auto_generated?.source === 'catalog-repair-v1' && (event.skills.every(g => (employee.skills[g.skill_id] || 0) >= g.max_level) || state?.history.some(h => h.employee_id === employee.employee_id && h.event_id === event.event_id && h.status === 'completed'))) return false;
  const prereqs = Object.entries(event.prerequisites || {}).every(([id, level]) => (employee.skills[id] || 0) >= level);
  const scheduled = !state || (!event.recurring && (!event.format || event.format === 'self_paced')) || remainingSessions(event, employee, state).length > 0;
...
export function recommend(state, employee) {
  ...
  const history = analyticalHistory(state).filter(h => h.employee_id === employee.employee_id);
  ...
  }).filter(e => e.closed > 0 && !e.repeat).sort((a, b) => Number(Boolean(a.auto_generated)) - Number(Boolean(b.auto_generated)) || b.score - a.score || a.event_id.localeCompare(b.event_id));
}
export function completeActivity(state, employee, eventId, now = asOf(state)) {
  ...
  const sessionDate = enrollment.session_date || (event.recurring ? enrollment.date?.slice(0, 10) || now.slice(0, 10) : undefined);
  const key = completionKey(event, { employee_id: employee.employee_id, event_id: eventId, session_date: sessionDate, date: now });
  if (event.recurring && state.history.some(h => h.status === 'completed' && completionKey(event, h) === key)) throw new Error('This session has already been completed.');
  const changes = skillChanges(event, employee);
  for (const change of changes) employee.skills[change.skill_id] = change.after;
  state.history.push({ history_id: `H-${crypto.randomUUID()}`, employee_id: employee.employee_id, event_id: eventId, status: 'completed', date: now, session_date: sessionDate, on_time: null, completion_pct: 100, changes, source: 'self_reported_demo' });
...
export function hrSummary(state, now = Date.parse(asOf(state))) {
  ...
  const people = state.employees.map(employee => {
    const hist = history.filter(h => h.employee_id === employee.employee_id);
  ...
  const recent = history.filter(h => now >= new Date(h.date).getTime() && now - new Date(h.date).getTime() <= 90 * 86400000);
...
export function parseCsv(text) {
  ...
  if (headers.some(key => !key)) throw new Error('CSV has empty column names.');
...
export function validateState(state) {
  ...
  for (const e of state.events) {
    ...
    for (const field of ['mandatory', 'recurring']) assert(e[field] === undefined || typeof e[field] === 'boolean', `${e.event_id}: ${field} must be boolean.`);
  ...
  for (const h of state.history) {
    ...
    if (h.session_date !== undefined) assert(typeof h.session_date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(h.session_date) && Number.isFinite(Date.parse(h.session_date)), 'Invalid history session_date.');
  ...
  assert(state.enrollments === undefined || Array.isArray(state.enrollments), 'Invalid enrollments.');
  for (const row of state.enrollments || []) {
    assert(row && employees.has(row.employee_id) && events.has(row.event_id), 'Invalid enrollment reference.');
    const key = `${row.employee_id}:${row.event_id}`;
    assert(!enrolled.has(key), 'Duplicate enrollment.'); enrolled.add(key);
    for (const field of ['date', 'session_date']) if (row[field] !== undefined) assert(typeof row[field] === 'string' && Number.isFinite(Date.parse(row[field])), 'Invalid enrollment date.');
  }
  assert(state.withdrawn_enrollments === undefined || (Array.isArray(state.withdrawn_enrollments) && state.withdrawn_enrollments.every(key => typeof key === 'string' && key.split(':').length === 2 && employees.has(key.split(':')[0]) && events.has(key.split(':')[1]) && !enrolled.has(key))), 'Invalid withdrawn enrollments.');
  return state;
}
export function exportBackup(state) {
  return { kind: 'career-quest-backup', backup_version: 1, state: structuredClone({ ...state, enrollments: state.enrollments || [], withdrawn_enrollments: state.withdrawn_enrollments || [] }) };
}
export function restoreBackup(payload) {
  assert(payload?.kind === 'career-quest-backup' && payload.backup_version === 1, 'Unsupported backup version.');
  const candidate = structuredClone(payload.state);
  assert(candidate && Array.isArray(candidate.enrollments) && Array.isArray(candidate.withdrawn_enrollments), 'Backup must include the complete personal plan.');
  // A backup restores the exact saved levels and plans; never replay its history.
